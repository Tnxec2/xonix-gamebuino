...
void move() {
  ...
  if ( board[player.x][player.y] == CELL_EMPTY ) board[player.x][player.y] = CELL_LINE;
  
  // move player
  if (player_velocity.x == 1 && player.x < BOARDWIDTH-1 ) player.x++;
  else if (player_velocity.x == -1 && player.x > 0 ) player.x--;
  if (player_velocity.y == 1 && player.y < BOARDHEIGHT-1 ) player.y++;
  else if (player_velocity.y == -1 && player.y > 0 ) player.y--;

  bool playerAttacked = false;
  // move enemys
  for (int i = 0; i < enemysCount; i++) {
    if ( board[enemys[i].x + enemys_velocity[i].x][enemys[i].y] == CELL_CLAIMED ) enemys_velocity[i].x = -(enemys_velocity[i].x);
    if ( board[enemys[i].x][enemys[i].y + enemys_velocity[i].y] == CELL_CLAIMED ) enemys_velocity[i].y = -(enemys_velocity[i].y);
    enemys[i].x += enemys_velocity[i].x;
    enemys[i].y += enemys_velocity[i].y;

    if ( player.x == enemys[i].x && player.y == enemys[i].y ) playerAttacked = true;
    if ( board[enemys[i].x][enemys[i].y] == CELL_LINE ) playerAttacked = true;
  }
...
void clearLine() {
  for (int row = 0; row < BOARDHEIGHT; row++) {
    for (int col = 0; col < BOARDWIDTH; col++) {
      if (board[col][row] == CELL_LINE ) board[col][row] = CELL_EMPTY;
    }
  }
}
...
void claimBoard() {
  ...
  for (int row = 0; row < BOARDHEIGHT; row++) {
    for (int col = 0; col < BOARDWIDTH; col++) {
      if (board[col][row] == CELL_LINE ) board[col][row] = CELL_CLAIMED;
      else if (board[col][row] == CELL_EMPTY ) board[col][row] = CELL_CAN_CLAIM;
    }
  }
  ...
  while (changed) {
    ...
    for (int row = 0; row < BOARDHEIGHT; ++row) {
      for (int col = 0; col < BOARDWIDTH; ++col) {
        if (board[col][row] != CELL_CAN_CLAIM) continue;
        ...
        if ((row > 1 && board[col][row - 1] == CELL_EMPTY) ||
          ...
          if (board[col][row] == CELL_CAN_CLAIM) board[col][row] = CELL_EMPTY;
          changed = true;
        }
      }
    }
  ...
  for (int row = 0; row < BOARDHEIGHT; row++) {
    for (int col = 0; col < BOARDWIDTH; col++) {
      if (board[col][row] == CELL_CAN_CLAIM ) board[col][row] = CELL_CLAIMED;
      if (board[col][row] == CELL_CLAIMED ) score++;
    }
  }
